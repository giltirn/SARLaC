#ifndef _FIT_PIPI_COMOVING_GPARITY_FITFUNC_H
#define _FIT_PIPI_COMOVING_GPARITY_FITFUNC_H
...
inline std::string getStub(const Operator op){
  switch(op){
  case Operator::PiPiComoveGnd:
    return "pipi_gnd";
  case Operator::PiPiComoveExc1:
    return "pipi_exc1";
  case Operator::PiPiComoveExc2:
    return "pipi_exc2";
  }
}
...
#define DEF(NM, GUESS) { param_map[NM] = p++;  guesses.push_back(GUESS); }  
  ...
  }else if(fitfunc == FitFuncType::FSimGenMultiState || fitfunc == FitFuncType::FSimGenMultiStateSub || fitfunc == FitFuncType::FSimGenMultiStateTminSub || fitfunc == FitFuncType::FSimGenMultiStateTminSubForceZero){ 
