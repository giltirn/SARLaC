...
void generateResampledData(jackknifeCorrelationFunctionD &j_data_gnd_gnd, doubleJackknifeCorrelationFunctionD &dj_data_gnd_gnd, 
			   jackknifeCorrelationFunctionD &j_data_exc_exc, doubleJackknifeCorrelationFunctionD &dj_data_exc_exc, 
			   jackknifeCorrelationFunctionD &j_data_gnd_exc, doubleJackknifeCorrelationFunctionD &dj_data_gnd_exc,
  ...
  if(opt.load_combined_data){
    ...
    rawDataCorrelationFunctionD const* raw[3] = { &raw_data_gnd_gnd, &raw_data_exc_exc, &raw_data_gnd_exc };
    jackknifeCorrelationFunctionD *dsets_j[3] = {  &j_data_gnd_gnd, &j_data_exc_exc, &j_data_gnd_exc};
    doubleJackknifeCorrelationFunctionD *dsets_dj[3] = {  &dj_data_gnd_gnd, &dj_data_exc_exc, &dj_data_gnd_exc};

    //Bin/resample
    for(int i=0;i<3;i++){
      *dsets_j[i] = binResample<jackknifeCorrelationFunctionD>(*raw[i], bin_size);
      *dsets_dj[i] = binResample<doubleJackknifeCorrelationFunctionD>(*raw[i], bin_size);
    }
    ...
    if(do_vacuum_subtraction){
      ...
      for(int i=0;i<3;i++){
	*dsets_j[i] = *dsets_j[i] - computePiPi2ptVacSub<jackknifeCorrelationFunctionD>(*bub[i], bin_size, tsep_pipi, proj_src[i], proj_snk[i]);
	*dsets_dj[i] = *dsets_dj[i] - computePiPi2ptVacSub<doubleJackknifeCorrelationFunctionD>(*bub[i], bin_size, tsep_pipi, proj_src[i], proj_snk[i]);
      }
    }
