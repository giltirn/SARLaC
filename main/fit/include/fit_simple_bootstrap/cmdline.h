#ifndef _FIT_SIMPLE_BOOTSTRAP_CMDLINE_H
#define _FIT_SIMPLE_BOOTSTRAP_CMDLINE_H
...
struct CMDline{
  ...
  std::string save_covariance_matrix_file;
  ...
  void setup(const int argc, const char** argv, const int begin = 0){
	...
	save_boot_resample_table_file = sargv[i+1];
      	i+=2;
      }else if(sargv[i] == "-save_covariance_matrix"){
	save_covariance_matrix = true;
	save_covariance_matrix_file = sargv[i+1];
	i+=2;
      }else{
