...
#pragma omp parallel for
  ...
  //Prepare file streams for outputting true ensembles
  std::vector<std::unique_ptr<std::ofstream> > true_ens_outfiles(omp_get_max_threads());
  if(cmdline.write_true_ens_data){
    for(int i=0;i<true_ens_outfiles.size();i++){
      true_ens_outfiles[i].reset(new std::ofstream("true_ens_data."+std::to_string(i)));
      true_ens_outfiles[i]->precision(14);
    }
  }
...
#pragma omp parallel for schedule(static)
  for(int test=0;test<ntest;test++){
    ...
    if(cmdline.write_true_ens_data){
      ...
      auto &of = *true_ens_outfiles[omp_get_thread_num()];
      for(int t=0;t<Lt;t++){
	of << t;
	for(int s=0;s<nsample;s++)
	  of << " " << data.value(t).sample(s);
	of << std::endl;
      }
      of << std::endl; //newline between ensembles generated by a particular thread
    }   
