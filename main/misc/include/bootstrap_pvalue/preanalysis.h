...
//Analyze for bias in the mean of q^2 over block resampled ensembles vs the q^2 of the original ensemble
//(no recentering)
struct preAnalysisBlockBootstrapQ2Bias : public preAnalysisBase{
  ...
  void run(const Args &args, const std::string &params_file, const covMatStrategyBase &covgen, const randomDataBase &datagen, genericFitFuncBase &fitfunc) const override{   
    ...
    for(int e=0;e<norig_ens;e++){
      std::vector<std::vector<int> > rtable = generateResampleTable(nsample, ntest, args.bootstrap_strat, args.block_size, threadRNG);
      correlationFunction<double, rawDataDistributionD> orig_data = datagen.generate(Lt,nsample);
      correlationFunction<double, double> data_means(Lt); for(int t=0;t<Lt;t++) data_means.value(t) = orig_data.value(t).mean();
      double q2_orig;
      {
	simpleSingleFitWrapper fitter(fitfunc, MinimizerType::MarquardtLevenberg, args.MLparams);
	...
	assert(fitter.fit(params,q2_orig,q2_per_dof,dof,data_means));
      }

      double q2_means_avg = 0.;
#pragma omp parallel for
      for(int test=0;test<ntest;test++){
	correlationFunction<double, rawDataDistributionD> rdata(Lt);
	for(int t=0;t<Lt;t++) rdata.value(t) = resampledEnsemble(orig_data.value(t),test,rtable);
	correlationFunction<double, double> data_means(Lt); for(int t=0;t<Lt;t++) data_means.value(t) = rdata.value(t).mean();
	simpleSingleFitWrapper fitter(fitfunc, MinimizerType::MarquardtLevenberg, args.MLparams);
	covgen.compute(fitter, rdata);
	parameterVector<double> params(fitfunc.Nparams(),0.);
	double q2, q2_per_dof; int dof;
	assert(fitter.fit(params,q2,q2_per_dof,dof,data_means));
    ...
    std::string fname = "bootstrap_q2_bias";
    plot_bias.write(fname + ".py",fname + ".pdf");
  }
};
