...
template<typename DistributionType>
NumericTensor<DistributionType,1> computeQamplitude(const int q, const int tsep_k_pi, const RawKtoPiPiData &raw, const BubbleData &bubble_data, const Args &args, const CMDline &cmdline, const std::string &descr){
  ...
  computeAlphaAndVacuumSubtractions(alpha_r, A0_type4_srcavg_vacsub_r, mix4_srcavg_vacsub_r,
				    raw.A0_type4_alltK_nobub, raw.mix4_alltK_nobub, getResampledBubble<DistributionType>::get(bubble_data),q, raw.nonzerotK(4),tsep_k_pi,args);
  ...
  for(int i=1;i<=4;i++) A0_srcavg_r(i) = resampleAverageTypeData<DistributionType>(raw.A0_alltK(i), q, raw.nonzerotK(i), args); //[t]
  for(int i=3;i<=4;i++) mix_srcavg_r(i) = resampleAverageMixDiagram<DistributionType>(raw.mix_alltK(i), raw.nonzerotK(i), args);
...
void getData(std::vector<correlationFunction<amplitudeDataCoord, jackknifeDistributionD> > &A0_all_j, 
	     ...
	     const BubbleData &bubble_data, const int tsep_k_pi_idx, const Args &args, const CMDline &cmdline){
  ...
  for(int q=0;q<10;q++){
    ...
    NumericTensor<doubleJackknifeDistributionD,1> A0_full_srcavg_dj = computeQamplitude<doubleJackknifeDistributionD>(q, tsep_k_pi, raw, bubble_data, args, cmdline, "double jackknife");
    NumericTensor<jackknifeDistributionD,1> A0_full_srcavg_j = computeQamplitude<jackknifeDistributionD>(q, tsep_k_pi, raw, bubble_data, args, cmdline, "single jackknife");
