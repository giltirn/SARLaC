...
struct CoordTview{
  ...
  inline double operator[](const int i) const{ //interface that FitFuncLinearMultiDim wants
    assert(i==0);
    return c.t;
  }
};
...
private:
  FitFuncLinearMultiDim<CoordTview,double,1, ParamView> fitfunc_a; //a + b*t
  FitFuncLinearMultiDim<CoordTview,double,1, ParamView> fitfunc_b; //c + d*t
public:
